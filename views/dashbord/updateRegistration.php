...
if (isset($_GET['section_id'])) {
  ...
  $json_student_register_by_session = $controller->studentRegisterBySession($section_id);
        ...
        <div>
          <select name="programRegistrationId" id="programRegistrationId" onchange="getProgramRegistrationId()" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
            ...
            <?php foreach ($programs as $programdata) : ?>
              <option value="<?= htmlspecialchars($programdata['program_name']); ?>" data-programRegistrationId="<?= $programdata['id']; ?>">
                <?= htmlspecialchars($programdata['program_name']); ?>
                <?= htmlspecialchars($programdata['level_name']); ?>
              </option>
            <?php endforeach; ?>
          </select>
