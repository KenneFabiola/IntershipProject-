...
<div id="addProgramModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center hidden z-50">
  ...
  <div class="bg-white p-6 rounded-lg shadow-lg max-w-md w-full justify-between">
    ...
    <div class="justify-between flex text-sm font-meduim">
      <button type="button" id="closeAddModal" class="px-4 py-2 text-white bg-gradient-to-r from-gray-600 via-gray-600 to-gray-600 rounded-md hover:bg-gray-600 focus:outline focus-ring cursor-pointer" onclick="closeAddProgram()">Annuler</button>
      <input value="Enregister" type="submit" id="addProgramButton" name="addProgram" class="px-4 py-2 bg-gradient-to-r from-blue-600 via-blue-600 to-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus-ring cursor-pointer ">
    </div>
    </form>
  </div>
</div>
...
<div id="updateProgramModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center hidden z-50">
  ...
  <div class="bg-white p-6 rounded-lg shadow-lg max-w-md w-full justify-between">
    ...
    <div class="p-4 md:p-5">
      ...
      <span class="text-sm text-red-500" id="errorUpdate"></span>

        <div class="space-y-8">
          <div>
          <span class="text-sm" id="updateProgramNameError"></span>
            <input type="hidden" name="last_modified_by" id="" value="<?= $_SESSION['id'] ?>">
            <input type="hidden" name="updateProgramById" id="updateProgramById">
            <input type="text" id="updateProgramName" name="program_name" placeholder="program_name" class="bg-gray-50 mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring focus:border-blue-300" >
          </div>
          <div>
          <span class="text-sm" id="updateLevelNameError"></span>
            <input type="text" id="updateLevelName" name="level_name" placeholder="level_name" class="bg-gray-50 mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring focus:border-blue-300" >
          </div>
          <div>
          <span class="text-sm" id="updateProgramDescriptionError"></span>
            <input type="text" id="updateProgramDescription" name="descriptive" placeholder="describe" class="bg-gray-50 mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring focus:border-blue-300" >
          </div>
          <div>
          <span class="text-sm" id="updateProgramDurationError"></span>
            <input type="text" id="updateProgramDuration" name="duration" placeholder="duration" class="bg-gray-50 mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring focus:border-blue-300" >
          </div>
    ...
    <div class="justify-between flex text-sm font-meduim">
      ...
      <button type="button" id="closeAddModal" class="px-4 py-2 text-white bg-gradient-to-r from-gray-600 via-gray-600 to-gray-600 rounded-md hover:bg-gray-600 focus:outline focus-ring cursor-pointer" onclick="closeUpdateProgramModal()">Annuler</button>
      <input value="Enregister" type="submit" name="updateProgram" class="px-4 py-2 bg-gradient-to-r from-blue-600 via-blue-600 to-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus-ring cursor-pointer ">
    </div>
    </form>
  </div>
</div>
  ...
  <div class="bg-white p-6 rounded-lg shadow-lg max-w-md w-full justify-between">
    ...
    <div class="p-4 md:p-5">
      ...
      <h3 class="mb-5 text-lg font-normal text-gray-900 dark:text-gray-400">
        Voulez vous vraiment supprimer la filière <input readonly class="text-sm text-red-900" id="deleteProgramByName">?
      </h3>

      <form action="../../api/controllers/ProgramController.php" method="POST">
        <input readonly type="hidden" name="deleteProgramById" id="deleteProgramById" class="text-gray-900">
        <input value="Oui" href="" type="submit" name="deleteProgram" type="button" id="deleteProgram" class="text-black cursor:pointer rounded-lg border border-red-200  bg-gradient-to-r from-red-600 via-red-600 to-red-600 focus:ring-red-300 text-center py-2 px-2 hover:text-red-100">

        <button type="button" class="text-black cursor:pointer bg-gradient-to-r from-blue-600 via-blue-600 to-blue-600 py-2 px-2 ms-3 text-sm font-meduim text-gray-900 focus:outline-none rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:ring-4 focus:ring-gray-100" onclick="closeDeleteProgram()">
          Non
        </button>
      </form>
